...
RUN apk add --no-cache mariadb-client libc6-compat libc-dev gcc busybox-extras bash mysql-client postgresql-dev \
        postgresql-client python3 python3-dev mariadb-dev npm libxslt-dev jpeg-dev
RUN pip3 install --no-cache-dir --upgrade pip
RUN pip3 install --no-cache-dir mysqlclient
RUN ln -s /usr/bin/python3 /usr/bin/python

RUN apk add gcc python3-dev libxml2-dev git alpine-sdk rsync
...
COPY ./configs/mysql_common.py.patch /app/djangoproject/settings/
...
RUN patch \
    ...
    /app/djangoproject/settings/mysql_common.py.patch
...
COPY ./mysql_entry.sh /app/docker/entry.sh
